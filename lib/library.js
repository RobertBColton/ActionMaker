...
export default {
	...
	serializeLIB(lib, ver) {
		...
		for (act of lib.actions) {
			...
			for (var k = act.args.length; k < 8; k++) {
			...
			var execType = act.execType;
			var execInfo = act.execInfo;
	...
	saveLGLIcons(callback, lib, columns = 0) {
		var actnum = 0;
		for (act of lib.actions) {
			var k = act.kind;
		...
		var rows = Math.ceil(actnum / columns);
	...
	serializeLGL(callback, lib, columns = 0) {
		const HDR = "LGL";
		const VER = 160;
		var encoder = new Encoder();
		...
		this.saveLGLIcons(function(blob) {
